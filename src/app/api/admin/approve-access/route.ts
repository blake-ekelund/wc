...
export async function GET(request: NextRequest) {
  ...
  return htmlResponse(
    ...
    `<p>A WorkChores admin is requesting temporary access to view the workspace <strong>${workspace?.name || "Unknown"}</strong>.</p>
     ...
     <form method="POST" action="${approveUrl}" style="margin-top:24px;">
...
function htmlResponse(title: string, body: string, type: "error" | "success" | "info" | "confirm") {
  ...
  return new NextResponse(
    `<!DOCTYPE html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${title} — WorkChores</title></head>
