...
import { Filter, ChevronRight, AlertCircle, Archive, Trash2, RotateCcw, CheckSquare, Square, X, GitBranch, UserCheck, ChevronDown, Mail, Loader2, AlertTriangle, Plus, Users, Search, RefreshCw } from "lucide-react";
          ...
          <div className="bg-white rounded-xl border border-border overflow-hidden">
            ...
            {filtered.length === 0 && (
              <div className="text-center py-16 px-6">
                ...
                <h4 className="text-sm font-semibold text-foreground mb-1">No contacts found</h4>
                <p className="text-xs text-muted max-w-xs mx-auto mb-4">{stageFilter !== "All" ? "No contacts in this stage. Try selecting a different filter." : "Add your first contact to get started."}</p>
                {stageFilter !== "All" && (
                  <button onClick={() => setStageFilter("All")} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-accent hover:text-accent-dark border border-accent/30 rounded-lg hover:bg-accent/5 transition-colors">
            ...
            {bulkEmailResult ? (
              ...
              <>
                ...
                <div className="px-6 py-4 space-y-3">
                  ...
                  {bulkEmailError && (
                    <div className="flex items-center justify-between gap-3 text-xs text-red-600 bg-red-50 rounded-lg px-3 py-2 border border-red-200">
                      <div className="flex items-center gap-2"><AlertTriangle className="w-3.5 h-3.5 shrink-0" />{bulkEmailError}</div>
                      <button onClick={() => setBulkEmailError("")} className="inline-flex items-center gap-1 px-2 py-1 text-[10px] font-medium text-red-700 hover:bg-red-100 rounded transition-colors shrink-0">
